  ...
  public:
	  ...
	  void run() {
		  ...
		  while (!isStopped()) {
			  ...
			  // ======== TASK: FILTERING OF DETECTED MOVING PIXELS USING MORPHOLOGICAL OPERATORS ========	
			  // dilate original image
			  cv::dilate(output, morphoFrame, cv::Mat(), cv::Point(-1,-1), 2);
			  // in-place erosion of the dilated image
			  cv::erode(morphoFrame, morphoFrame, cv::Mat(), cv::Point(-1,-1), 7);
			  
			  // ======== TASK: CONNECTED COMPONENT LABELLING (4-NEIGHBOURHOOD, 2 PASS METHOD) ========	
			  tempCCL = tracker.extractBlobs(morphoFrame);


			  // ======== TASK: DISPLAY OF VIDEO IMAGE PROCESSES ========
			  displayFourProcesses(frame, output, morphoFrame, tempCCL);
	...
	// --- DISPLAY 2x2 VIDEO PROCESSES ---
	void displayFourProcesses(Mat& upLeftImage, Mat& upRightImage, Mat& dnLeftImage, Mat& dnRightImage) {
		
		//Convert grayscale upRightImage to BGR upRightImage for compatibility

		//cvtColor(upLeftImage, upLeftImage, CV_GRAY2BGR);	
		cvtColor(upRightImage, upRightImage, CV_GRAY2BGR);	
		cvtColor(dnLeftImage, dnLeftImage, CV_GRAY2BGR);
		//cvtColor(dnRightImage, dnRightImage, CV_GRAY2BGR);

		Mat doubleImage(upLeftImage.rows*2, upLeftImage.cols*2, upLeftImage.type());

		upLeftImage.copyTo(doubleImage(Rect(0, 0, upLeftImage.cols, upLeftImage.rows)));
		upRightImage.copyTo(doubleImage(Rect(upLeftImage.cols, 0, upLeftImage.cols, upLeftImage.rows)));
		dnLeftImage.copyTo(doubleImage(Rect(0, upLeftImage.rows, upLeftImage.cols, upLeftImage.rows)));
		dnRightImage.copyTo(doubleImage(Rect(upLeftImage.cols, upLeftImage.rows, upLeftImage.cols, upLeftImage.rows)));

		namedWindow("Assignment 3", CV_WINDOW_AUTOSIZE);
		imshow("Assignment 3", doubleImage);
	}
};
